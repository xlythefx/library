...
export function SettingList() {
  ...
  const [meta, setMeta] = useState<Paginated<Setting>["meta"] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    try { const res = await apiGet<Paginated<Setting>>(`/settings?page=${p}`); setRows(res.data); setMeta(res.meta); }
  ...
  const remove = async (id: number) => {
    ...
    await apiDelete(`/settings/${id}`); load(page);
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link to="/settings/new" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium"><Plus className="w-4 h-4" /> New Setting</Link>
      </div>
      ...
      {!loading && !error && (
        <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={3} className="text-center px-4 py-10 text-slate-500">No settings yet.</td></tr>}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-300 font-medium">{row.key}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{row.value}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/settings/${row.id}`)} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
