...
export function BookList() {
  ...
  const [meta, setMeta] = useState<Paginated<Book>["meta"] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    setError(null);
    try {
      const res = await apiGet<Paginated<Book>>(`/books?page=${p}`);
      setRows(res.data);
      setMeta(res.meta);
    } catch (e) {
      setError(String(e));
  ...
  const availBadge = (avail: number, total: number) => {
    const ratio = total > 0 ? avail / total : 0;
    const cls = ratio === 0 ? 'bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-400' :
      ratio < 0.5 ? 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400' :
      'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400';
    return <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${cls}`}>{avail}/{total}</span>;
  };

  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Books</h1>
        <Link
          to="/books/new"
        ...
        <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center px-4 py-10 text-slate-500">
              ...
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-500">{row.isbn}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{row.published_year ?? '—'}</td>
                  <td className="px-4 py-3">{availBadge(row.copies_available, row.copies_total)}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/books/${row.id}`)} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400" title="Edit">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => remove(row.id)} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40" title="Delete">
