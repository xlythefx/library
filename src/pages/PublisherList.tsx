...
type Publisher = { id: number; name: string; address: string | null; contact_email: string | null };
...
export function PublisherList() {
  ...
  const [meta, setMeta] = useState<Paginated<Publisher>["meta"] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    try { const res = await apiGet<Paginated<Publisher>>(`/publishers?page=${p}`); setRows(res.data); setMeta(res.meta); }
  ...
  const remove = async (id: number) => {
    ...
    await apiDelete(`/publishers/${id}`); load(page);
  };

  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Publishers</h1>
        <Link to="/publishers/new" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium"><Plus className="w-4 h-4" /> New Publisher</Link>
      </div>
      ...
      {!loading && !error && (
        <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={4} className="text-center px-4 py-10 text-slate-500">No publishers yet.</td></tr>}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{row.contact_email ?? '—'}</td>
                  <td className="px-4 py-3 text-slate-500 max-w-xs truncate">{row.address ?? '—'}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/publishers/${row.id}`)} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
