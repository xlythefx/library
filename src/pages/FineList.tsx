...
type Fine = { id: number; loan_id: number; member_id: number; amount: string; reason: string; is_paid: boolean; paid_at: string | null };
...
export function FineList() {
  ...
  const [meta, setMeta] = useState<Paginated<Fine>["meta"] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    try { const res = await apiGet<Paginated<Fine>>(`/fines?page=${p}`); setRows(res.data); setMeta(res.meta); }
  ...
  const remove = async (id: number) => {
    ...
    await apiDelete(`/fines/${id}`); load(page);
  };

  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Fines</h1>
        <Link to="/fines/new" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium"><Plus className="w-4 h-4" /> New Fine</Link>
      </div>
        ...
        <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400">
              <tr>
                <th className="text-left px-4 py-3 font-medium">Loan ID</th>
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={6} className="text-center px-4 py-10 text-slate-500">No fines yet.</td></tr>}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  <td className="px-4 py-3 font-mono text-xs text-slate-500">{row.loan_id}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{row.member_id}</td>
                  <td className="px-4 py-3 font-medium text-red-600 dark:text-red-400">${Number(row.amount).toFixed(2)}</td>
                  <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{row.reason}</td>
                  <td className="px-4 py-3">
                    {row.is_paid
                      ? <span className="inline-block px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">Paid</span>
                      : <span className="inline-block px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-400">Unpaid</span>
                    }
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/fines/${row.id}`)} className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
